...
make_pipeline(sg_pipeline_desc *desc) {
	...
	desc->label = "miru-test-rounded-rect-pipeline";
...
submit(void *m_, struct soluna_material_stream_context ctx, int n) {
	...
	for (i=0; i<out_n; i++) {
		...
		struct soluna_material_stream_basis size_data;
		struct soluna_material_stream_data shape_data;
		struct rounded_inst *inst = &tmp[i];
		if (!soluna_material_stream_read_basis(ctx, base, sizeof(size_payload), &size_payload, &size_data)) {
			return;
		}
		if (!soluna_material_stream_read(ctx, base + 1, sizeof(shape_payload), &shape_payload, &shape_data)) {
			return;
		}
		inst->rect[0] = size_data.basis.origin.x;
		inst->rect[1] = size_data.basis.origin.y;
		inst->rect[2] = size_payload.a;
		inst->rect[3] = size_payload.b;
		inst->basis[0] = size_data.basis.axis_x.x;
		inst->basis[1] = size_data.basis.axis_x.y;
		inst->basis[2] = size_data.basis.axis_y.x;
		inst->basis[3] = size_data.basis.axis_y.y;
...
lmaterial_rounded_rect_submit(lua_State *L) {
	struct material_rounded_rect *m = (struct material_rounded_rect *)luaL_checkudata(
		L,
		1,
		"MIRU_TEST_MATERIAL_ROUNDED_RECT"
	);
...
init_pipeline(struct material_rounded_rect *m) {
	sg_pipeline_desc desc = {
		.layout.attrs = {
			...
			[ATTR_rounded_rect_basis].format = SG_VERTEXFORMAT_FLOAT4,
...
lnew_material_rounded_rect(lua_State *L) {
	...
	if (luaL_newmetatable(L, "MIRU_TEST_MATERIAL_ROUNDED_RECT")) {
		luaL_Reg l[] = {
			{ "__index", NULL },
